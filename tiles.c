...
/*   Updated: 2023/09/09 20:57:02 by azaaza           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_put_image_to_window(game->mlx, game->win, game->tiles.grass, col
			* TILE_SIZE, row * TILE_SIZE);
	if (row == 0 || row == game->map.rows - 1)
	{
		mlx_put_image_to_window(game->mlx, game->win,
				game->tiles.vertical_fence, col * TILE_SIZE, row * TILE_SIZE);
	}
	else if (col == 0 || col == game->map.columns - 1)
	{
		mlx_put_image_to_window(game->mlx, game->win,
				game->tiles.horizontal_fence, col * TILE_SIZE, row * TILE_SIZE);
	}
	else
	{
		mlx_put_image_to_window(game->mlx, game->win, game->tiles.bush, col
				* TILE_SIZE, row * TILE_SIZE);
	}
}
...
void	load_tiles(t_game *game)
{
	...
	game->tiles.horizontal_fence = mlx_xpm_file_to_image(
		game->mlx, "./assets/world/rock.xpm", &width, &height);
	game->tiles.bush = mlx_xpm_file_to_image(game->mlx,
												"./assets/world/rock.xpm",
												&width,
												&height);
	game->tiles.grass = mlx_xpm_file_to_image(
		game->mlx, "./assets/world/grass.xpm", &width, &height);
	game->tiles.dirt = mlx_xpm_file_to_image(game->mlx,
												"./assets/world/floor.xpm",
												&width,
												&height);
}
